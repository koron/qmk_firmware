...
static const char BL = '\xC0'; // Blank indicator character
static const char LFSTR_ON[] PROGMEM = "\xCB\xCC";
static const char LFSTR_OFF[] PROGMEM = "\xCD\xCE";
...
void keyball_oled_render_ballinfo(void) {
    ...
    oled_write_P(PSTR("Ball\xBF"), false);
    ...
    oled_write_P(PSTR("    \xBF\xC5\xC6"), false);
    oled_write(format_4d(keyball_get_cpi()) + 1, false);
    oled_write_P(PSTR("00 "), false);

    // indicate scroll mode: on/off
    oled_write_P(PSTR("\xC7\xC8"), false);
    ...
    oled_write_P(PSTR(" \xC9\xCA"), false);
...
void keyball_oled_render_keyinfo(void) {
    ...
    // "Key" Label
    oled_write_P(PSTR("Key \xBF"), false);

    // Row and column
    oled_write_char('\xC1', false);
    oled_write_char(to_1x(keyball.last_pos.row), false);
    oled_write_char('\xC2', false);
    oled_write_char(to_1x(keyball.last_pos.col), false);

    // Keycode
    oled_write_P(PSTR("\xC3\xC4"), false);
...
void keyball_oled_render_layerinfo(void) {
    ...
    //     Layer:-23------------
    //
    oled_write_P(PSTR("L\xD1\xD2r\xBF"), false);
...
#    ifdef POINTING_DEVICE_AUTO_MOUSE_ENABLE
    oled_write_P(PSTR("\xCF\xD0"), false);
...
#    else
    oled_write_P(PSTR("\xCF\xD0\xCD\xCE ---"), false);
#    endif
#endif
}
